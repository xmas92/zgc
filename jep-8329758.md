...
We propose to enhance ZGC so that, when Automatic Heap Sizing is enabled with `-XX:+ZAdaptive`, the JVM:
...
Additionally, on supported Linux platforms, the JVM allocates memory more efficiently, in units of 2MB rather than 4KB by default.
...
```
java -XX:+UseZGC -XX:+ZAdaptive -jar app.jar
```
...
ZGC startup defaults have changed over time, but deployers still face the same tradeoff between a smaller initial heap for fast startup and a larger initial heap for smoother warmup.
...
```
java -XX:+UseZGC -XX:+ZAdaptive -jar warmup.jar
```
