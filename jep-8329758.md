...
We propose that ZGC starts with only a small initial heap. Since committing memory is more than mere bookkeeping for the OS, it can be slow, so committing as little as possible makes startup as fast as possible. ZGC uses rapid expansion, discussed next, to adapt the heap size to the true allocation needs of the application.
