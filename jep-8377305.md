...
- Introduce a new adaptive heap sizing policy that requires minimal configuration.
...
We propose an Automatic Heap Sizing mode for ZGC that, when enabled with `-XX:+ZAutomaticHeapSizing`:
...
```
java -XX:+UseZGC -XX:+ZAutomaticHeapSizing …
```
...
In a future release we intend to make Automatic Heap Sizing the default.
...
(( ZGC adapts the heap size to approach a target GC intensity – represented by the amount of CPU used by the GC as a fraction of the amount of CPU used by the application – which is a tradeoff between the amount of memory currently used for the heap (capacity) and the amount of CPU used to perform GC work. By default, a target GC intensity is chosen to strike a balance between memory usage and the amount of CPU used by the GC. ))
...
ZGC dynamically varies an internal heap target size to be a good neighbor to other processes on the system and effectively trade off between CPU and memory use.

ZGC’s default target GC intensity is 12.5% of the application’s CPU use. We believe this is a reasonable balance of CPU use and memory use.
...
```
java -XX:+UseZGC -XX:+ZAutomaticHeapSizing -XX:ZGCIntensity=4 -jar app.jar
```
