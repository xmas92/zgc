...
To alter the CPU usage, you configure the _intensity_ of garbage collection via a command line option. Intensity is an integer between 1 and 10, with a default of 5. Higher values make ZGC more intensive, resulting in more frequent collections, higher CPU usage, and smaller heap sizes; lower values make ZGC less intensive, triggering less frequent collections but requiring a larger heap. For example, to use more memory in exchange for more CPU available to the application:
