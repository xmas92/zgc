...
static double cpu_latency_factor(size_t c, double rho, double unluckyness) {
  double prob_join_queue = ZErlangCLUT::get(c, rho);
...
static double compute_cpu_vs_latency_pressure(const ZSystemCpuPressureMetrics& machine_cpu_metrics) {
  ...
  const double p99_response_time_scaling = cpu_latency_factor((size_t)os::Machine::active_processor_count(), rho, 100.0);
